...
export default function Page() {
  ...
  const left = `var(--sidebar-width, ${isCollapsed ? COLLAPSED_WIDTH : EXPANDED_WIDTH}px)`;
  const width = `calc(100% - var(--sidebar-width, ${isCollapsed ? COLLAPSED_WIDTH : EXPANDED_WIDTH}px))`;

  return (
    <>
      <nav
        className="fixed top-0 bg-[#0b0e14]/80 backdrop-blur-2xl border-b border-white/5 h-20 overflow-visible transition-all duration-300"
        style={{ left, right: 0, width, zIndex: 900 }}
      >
        <div className="max-w-[1440px] mx-auto flex items-center justify-between h-full px-8">
          ...
          <div className="hidden md:flex flex-1 justify-center min-w-0">
            <div className="flex gap-8 items-center min-w-0">
              <Link href="#" className="text-slate-400 font-medium hover:text-white transition-colors truncate">Ecosistema</Link>
              <Link href="#" className="text-slate-400 font-medium hover:text-white transition-colors truncate">Laboratorio</Link>
              <Link href="#" className="text-slate-400 font-medium hover:text-white transition-colors truncate">Análisis</Link>
              <Link href="#" className="text-slate-400 font-medium hover:text-white transition-colors truncate">Arquitectura</Link>
            </div>
          </div>

          {/* Header: removed search, notifications and Construir Proyecto per request */}
        </div>
      </nav>
